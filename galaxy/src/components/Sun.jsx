...
const Sun = ({handPos, isPinching,selectedPlanet,setSelectedPlanet,planetId,dragOffset,setDragOffset}) => {
  const [position,setPosition] = useState({x: window.innerWidth/2*0.2, y:window.innerHeight/2*0.3})
  ...
  return (
    <div>
      <div ref = {planetRef}
      className={`sun rotate ${selectedPlanet === planetId ? 'selected': ' '}  `}
      style = {{
        position: 'absolute',
        left:position.x,
        top: position.y,
      ...
      >
       <img className='sunimg' src="https://plus.unsplash.com/premium_photo-1670210080045-a2e0da63dd99?fm=jpg&q=60&w=3000&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MXx8c3VuJTIwaW4lMjBzcGFjZXxlbnwwfHwwfHx8MA%3D%3D" alt="" />
      </div>
    </div>
  )
}
