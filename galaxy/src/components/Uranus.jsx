...
const Uranus = ({handPos, isPinching,selectedPlanet,setSelectedPlanet,planetId,dragOffset,setDragOffset}) => {
  const [position,setPosition] = useState({x: window.innerWidth/2*0.7, y:window.innerHeight/2*0.7})
  ...
  return (
    <div>
      <div ref = {planetRef}
      className={`uranus rotate ${selectedPlanet === planetId ? 'selected': ' '}  `}
      style = {{
        position: 'absolute',
        left:position.x,
        top: position.y,
      ...
      >
        <img  src="https://thumbs.dreamstime.com/b/planet-uranus-space-planet-uranus-starry-sky-solar-system-space-image-elements-furnished-nasa-174840122.jpg" alt="" className="uranusimg" />
      </div>
    </div>
  )
}
