...
const Jupiter = ({handPos, isPinching,selectedPlanet,setSelectedPlanet,planetId,dragOffset,setDragOffset}) => {
  const [position,setPosition] = useState({x: window.innerWidth/2*0.2, y:window.innerHeight/2})
  ...
  return (
    <div>
      <div ref = {planetRef}
      className={`jupiter rotate ${selectedPlanet === planetId ? 'selected': ' '}  `}
      style = {{
        position: 'absolute',
        left:position.x,
        top: position.y,
      ...
      >
        <img src="https://media.istockphoto.com/id/1009152604/photo/jupiter-planet-black-background.jpg?s=170667a&w=0&k=20&c=wLtiVGo6PY3FmWtaDLHLQnWgqOnJhLjzQFx3nJeHrX0=" alt="" className="jupiterimg" />
      </div>
    </div>
  )
}
